...
LDFLAGS_WINDOWS = lib/libraylib.a -lopengl32 -lgdi32 -lwinmm
...
ifeq ($(OS),Linux)
    ...
    EXT =
    ARCHIVE_CMD = tar -czf arkanoid-linux.tar.gz $(OUT) README.md LICENSE
else
    LDFLAGS = $(LDFLAGS_WINDOWS)
    EXT = .exe
    ARCHIVE_CMD = zip arkanoid-windows.zip $(OUT) README.md LICENSE
endif
